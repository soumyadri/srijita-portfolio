...
export default function AboutSection({ aboutMe, whatIDo }: any) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {whatIDo.map((item: any, index: number) => (
          <div key={item.id} className={`${isDarkMode ? index % 3 === 0 ? 'bg-[#0e2f39]' : 'bg-[#3b4314]' : index % 3 === 0 ? 'bg-[#d8f8ff]' : 'bg-[#fbffe7]'} p-4 rounded shadow-md my-4 mx-2`}>
            ...
            <div className={`${isDarkMode ? 'text-[#e0dddd]' : 'text-[#2a2929]'} overflow-ellipsis overflow-hidden h-24 line-clamp-4 text-justify`}>{item.description}</div>
          </div>
        ))}
      </div>
    </div>
